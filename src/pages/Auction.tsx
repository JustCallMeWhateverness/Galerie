...
export default function Auction() {
  ...
  const [auctionInformation, setAuctionInformation] =
    useState<AuctionInfo | null>(null);
  ...
  const { isFavorited, showAuthModal, onFavorite, setShowAuthModal } =
    useFavorite(isFavoritedByUser, auction);
  ...
  async function refreshBid() {
    try {
      ...
      if (!response.ok) {
        console.error(
          `Failed to refresh bids: ${response.status} ${response.statusText}`
        );
        return;
      }
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      try {
        ...
        if (!response.ok) {
          console.error(
            `Failed to fetch auction: ${response.status} ${response.statusText}`
          );
          return;
        }
        const data: AuctionResponse = await response.json();
        setAuctionInformation({
          ...
          seller:
            data.seller && data.seller.length > 0
              ? data.seller[0]
              : ({ username: "Unknown" } as Customer),
  ...
  function getBidContent() {
    ...
    if (now < time.startTime) {
      return (
        <Alert variant="warning">
          This auction starts at {time.startTime.toDateString()}
        </Alert>
      );
    }
      ...
      if (winningBid && winningBid.customerId === user.id) {
        return (
          <WinnerCard
            ...
            hasBeenPaid={hasBeenPaid ?? false}
            buttonPress={PayTheMan}
          />
        );
      }
    }
    return (
      <Alert variant="warning">
        This auction ended at {time.endTime.toDateString()}
      </Alert>
    );
  }
      ...
      {!isLoading && (
        <Row>
          <Col>
            ...
            <div>{getBidContent()}</div>

            {/* BidHistory is only visible after auction has started */}
            <div>
              {time && time.startTime < now ? <BidHistory bids={bids} /> : ""}
            </div>
          </Col>
        </Row>
      )}
